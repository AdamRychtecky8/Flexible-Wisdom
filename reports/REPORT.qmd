...
Pooling agents helps both populations, but the gains arrive early and then flatten (@fig-headline). Measured as accuracy added per extra agent, the marginal benefit falls below one percentage point by around five agents in every condition, for both humans and LLMs. There is no sharp cutoff; the curve simply bends. The full twelve-agent group is not meaningfully better than a group of five or seven. Removing the correlation between agents while holding each agent's accuracy fixed raises the curve: the observed ensemble sits below this independence baseline at every group size (@fig-independence), and that shortfall is the practical cost of correlated errors.
...
![Observed LLM ensemble accuracy (solid) versus an independence baseline (dashed) by group size, across the three conditions. The independence baseline applies the identical majority-vote procedure after independently permuting each agent's per-trial correctness, which preserves every agent's accuracy but removes the correlation between agents' errors. The observed curve sits below the independence baseline, and the shortfall is the practical cost of correlated errors. This comparison is descriptive and does not attribute the entire gap to correlation alone.](../outputs/ensemble-vs-independence.png){#fig-independence width=100%}
...
The early saturation has a mechanism. Agent errors are positively correlated: agents tend to fail on the same trials (@tbl-corr). The structure is visible directly in @fig-correlation: most pairs sit in the weak-to-moderate range, none are negative, and the strongest pair is the two readout methods of the same Gemini model. Across conditions, the mean pairwise error correlation is about 0.29, most pairs fall in the weak-to-moderate range, and no pairs are negatively correlated. Strong correlation (r above 0.5) is present but uncommon, in 8 to 14 percent of pairs depending on condition. Positive correlation of this size is enough to make a group of twelve worth less than twelve independent votes, which is consistent with the saturation seen above, although this report does not formally decompose the effective sample size.

![Pairwise error correlation across the twelve LLM agents, 80 percent cue-validity condition. The pattern is similar across conditions. Most pairs are weakly to moderately positively correlated, none are negative, and the most correlated cell is the two readout methods of the same Gemini model. This correlation structure is why ensemble gains saturate early.](../outputs/error-correlation-heatmap.png){#fig-correlation width=100%}
